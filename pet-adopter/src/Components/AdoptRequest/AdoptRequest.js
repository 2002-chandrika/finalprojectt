...
const AdoptRequest = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/api/v1/adoptrequest')
    ...
    const approveRequest = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.put(`http://localhost:8080/api/v1/adoptrequest?id=${id}`, { status: 'approved' })
                    // axios.put(`http://localhost:8080/api/v1/adoptrequest?id=${id}`)
                    .then(res => {
    ...
    const RejectRequest = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // axios.put(`http://localhost:8080/api/v1/adoptrequest?id=${id}`, { status: 'rejected' })
                axios.put(`http://localhost:8080/api/v1/adoptrequest?id=${id}`, { status: 'rejected' })
                    .then(res => {
                        if (res?.data?.acknowledged) {
                            router.refresh()
